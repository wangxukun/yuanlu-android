...
- [x] **Phase 2 - M9: 个人中心 + 头像上传（旅程数据/里程碑/账号与安全 + 编辑资料弹窗，复刻 Web /auth/personal-center；订阅部分仍待做）** (✅ 完成 2026-09-04，两端构建全绿 + 81 单测通过；同日按需求全平台下线「最近听过」tab——Web 同步移除该 tab 与 RecentHistory.tsx 组件，Android 连同 VM 状态/Repository 方法/AuthApi 端点/DTO/域模型整体删除，后端 /api/user/history/recent 路由保留供首页 SSR 复用其服务层)
  - **🚨 后端补齐移动端 Bearer 鉴权（yuanlu 仓库，4 个既有路由）**：`stats/overview`、`stats/weekly-activity`、`achievements`、`PUT user/profile` 原先仅走 Web Cookie 会话（`auth()`），Android Bearer 不可达——统一切换 `requireAuth()`（Cookie 优先不变，Bearer 兜底），Web 行为零影响。
  - **数据层**：`UserProfile` 域模型补 avatarFileName + 三项学习目标字段；新模型 `ProfileStats`/`WeeklyActivityItem`/`AchievementItem`/`RecentHistoryItem`；`ProfileDtos.kt` 五组 DTO；AuthApi 五端点（含 `@Multipart PUT api/user/profile` 与 Web FormData 同构，avatar 可空分片自动省略）；AuthRepository 五方法（multipart 构造 + errorMessage 三段 catch 模板）。
  - **UserProfileViewModel（Activity 作用域，Navigation 根创建）**：统管头部资料/旅程统计/周活动（本周↔上周切换）/成就 + 编辑表单态；保存成功 merge 旧资料的 email/phone/role（PUT 响应无 User 嵌套）、`profileRevision` 自增——「我的」Tab 监听 revision 自动刷新用户卡（对齐 Web updateSession→fetchProfile）。
  - **主页（PersonalCenterScreen，新增 NavKey：`PersonalCenterNav`，「我的」菜单「个人中心」进入）**：头部用户卡（88dp 白描边头像 + 昵称 + 曙光橙「远行客·等级」badge + 座右铭回退 + 加入日期/属地）→ 三选项卡（下划线指示器 + fade/slide 切换）：**旅程数据**＝三统计卡（累计里程=时长×5km 一位小数/连续天数/词汇路标，ShimmerBox 骨架屏）+ 周活动面积图（Canvas 中点贝塞尔平滑 + 渐变填充 15%→0% + 白描边数据点 + TextMeasurer 轴刻度，本周/上周分段切换）；**里程碑**＝远路路图（Canvas：蜿蜒贝塞尔虚线 + PathMeasure 按里程实线裁剪 + 5 节点点亮/灰暗 + 冲线小旗 + 进度脉冲点 2s 循环）+ 成就墙（4 列网格前 8 枚，已解锁远青底/未解锁灰暗半透明，标题计数 + 全部查看占位）；**账号与安全**＝手机号/邮箱（脱敏 138****8000、a****@domain + 已验证绿标/未绑定警示 + 绑定胶囊占位）/登录密码三列表卡。
  - **编辑资料 BottomSheet（EditProfileSheet）**：渐变头 + 关闭；双 Tab 胶囊（个人资料/学习目标）——**个人资料**＝头像选择（Photo Picker 无权限 → `cropSquareJpeg` 程序化居中方形裁剪 + 512px 下采样 + JPEG88，相机角标 + 字节预览，交互式裁剪的占位实现）+ 昵称（≤20 字校验 + 计数器 + 即时清错）+ 英语水平四档 FilterChip + 个性签名（≤100 字校验）；**学习目标**＝连胜提示横幅 + 三滑杆（每日 10–120min 步进 5 / 每周 1–20h 步进 1（后端 Int 存储，未复刻 Web 0.5 步进被 parseInt 截断的缺陷）/ 每周 10–200 词步进 5，值胶囊 + 三段刻度标注）；保存调 PUT 接口 → 回写资料 + revision 自增 + Toast「设置已更新」，失败保持弹窗并透出文案。
  - **纯逻辑抽离 `ProfileUtils.kt`**（JVM 可测）：KM_PER_HOUR=5 里程换算/formatKm（一位小数 US Locale）/formatHours（整数去小数点，JS 渲染口径）/levelLabel（Beginner→初级等，null→中级对齐 Web fetchProfile 默认）/displayName（昵称→邮箱前缀→User）/displayBio（空→座右铭）/maskPhone·maskEmail·isPlaceholderEmail（Web 正则同口径）/validateNickname·validateBio/sortAchievements（解锁优先稳定排序）/chartYMax（30 倍数向上取整、下限 60）/formatDate（ISO→yyyy/MM/dd）。
  - **验证**：Android `compileDebugKotlin`/`assembleDebug`/`testDebugUnitTest` 81/81（新增 `ProfileUtilsTest` 16 例 + `UserProfileViewModelTest` 10 例：加载/错误/周切换/表单回填默认值/昵称简介校验拦截/保存合并 revision/失败保弹窗/头像版本号；既有 LoginViewModelTest·ProfileViewModelTest 的 Fake 同步补新接口桩）；Web `tsc --noEmit` 零错误；**真机端到端实测**（华为 LYA-AL00，`adb reverse tcp:3000` 隧道 + `local.properties release.baseUrl=http://127.0.0.1:3000/` 指向本机 dev server——为使 release 包可达明文回环，network_security_config 增加 127.0.0.1 单域 cleartext 例外）：管理员账号登录后逐 tab 核验——旅程数据三卡（12.5km/3天/2词）+ 面积图、里程碑路图（2 节点点亮带旗 + 脉冲点）+ 成就墙（4 格 2 亮）、账号安全（手机未绑定⚠️/邮箱已验证脱敏/密码已设置）、编辑资料弹窗（回填/双 Tab/滑杆步进/清空昵称被红字拦截 0/20）；无崩溃。
- [x] **Phase 2 - M7: 发音弱项本三页闭环（能力画像/闯关复习/达人榜，复刻 Web /library/pronunciation 族）** (✅ 完成 2026-09-07，两端构建全绿 + 144 单测通过 + 模拟器 E2E 实测)
  - **🚨 后端补齐移动端（yuanlu 仓库，2 处）**：①新增 `GET api/speech/notebook`（`requireAuth()` Bearer 兼容）——弱项本主页聚合接口，复刻 Web SSR 页数据组装：发音能力画像（speechProfileService 五维聚合 + CEFR）+ 薄弱音素统计（phonemeStats 均分升序）+ 弱项句子（Step A/B/C：曾低于分数线→取最新一次→仍低于才收录，封面签名；会员全量/非会员前 3 条试用切片 + totalErrors）；②`api/speech/leaderboard` 由 cookie-only `auth()` → `requireAuth()`（M3.6 同款 Bearer 缺口，原 Android 调用必 401）。
  - **数据层**：`SpeechModels` +`SpeechProfile`（radarDims 五维派生，round 同 Web toRadarData）/`PhonemeStat`/`WeakSentenceRecord`（toSubtitle=Web mockSubtitle 口径：subtitleId 兜底 recognitionid、subtitleEnd 缺省 start+3；toPracticeRecord 供"最近得分"匹配）/`SpeechNotebook`（lockedCount/weakestPhoneme/masteredPhonemeCount 派生）/`LeaderboardPeriod·Metric·Entry·MyRank·SpeechLeaderboard`；`SpeechDtos` 六组 DTO（Prisma 均值小数 roundToInt）；`SpeechApi` +`notebook`/`errors`/`leaderboard` 三端点；`SpeechRepository` 三方法（403→锁定态语义保留、401→"请先登录"文案）。
  - **模块一·弱项本主页**（`feature/pronunciation/PronunciationNotebookScreen`，新增 NavKey `PronunciationNotebookNav`，「我的」菜单进入）：发音能力画像卡（Canvas 自绘五维雷达 `RadarChart`——同心网格环+轴线+20% 填充多边形+TextMeasurer 维度标签，紫色 #7C3AED；CEFR 徽章；评测次数/综合得分/平均语速三格；CEFR 说明/引导文案；无数据空态）→ 统计面板（Mic 标题+副标题+曙光橙"发音达人榜"胶囊入口+待复习句子/最弱音素/已攻克音素三卡）→ 复习计划横幅（深青蓝 info-600，"N 个弱项句子"+"开始闯关复习"白按钮；空弱项显示"全部完成了！"）→ 薄弱音素诊断雷达（最弱前 6 项 /音素/ 标签，靛蓝 #4F46E5；<3 项"数据积累中"空态）→ 待复习弱项句子列表（封面+剧集标题+原句+得分徽章（≥80 绿/≥60 黄/其余红）+zh-CN 长日期；点击跳剧集详情；非会员锁定卡"还有 N 条待攻克"+解锁按钮）；下拉刷新 + 从闯关/达人榜返回自动重入刷新（entry 重进组合触发 `onReenter`）。
  - **模块二·闯关复习**（`WeaknessPracticeScreen`，`WeaknessPracticeNav`，横幅按钮进入）：顶栏"发音闯关复习 (i/N)"+"已达标"徽章 + 圆角进度条 + **复用 `SpeechEvalCard`**（AI朗读/原声/慢速/最近得分三钮区、逐词/翻译/音标/盲读文本区、大录音钮、结果环形分+逐词纠错+音素诊断）+ 底部上一题/下一题（末题变"完成复习"返回弱项本）。`WeaknessPracticeViewModel` 维护兼容 `SpeechEvalUiState` 的 card 子状态（subtitles=弱项伪字幕、audioUrl 随题切换）——每题独立 episodeId 提交 evaluate、WavRecorder 音量条、单一 MediaPlayer 互斥播放族（TTS/原声快慢/录音回放/词级四路）、达标题标记（≥weakThreshold）；403→整页 PRO 锁定态（锁图标+解锁按钮）、空弱项→"没有待复习的弱项"完成态。
  - **模块三·达人榜**（`SpeechLeaderboardScreen`，`SpeechLeaderboardNav`，主页胶囊入口）：顶栏奖杯标题+规则副标题（"榜单按平均综合分（≥5次评测）排名"/"榜单按练习次数排名"随维度联动）+ 双重 Tab（周期近7天/今日·主色胶囊；维度平均分榜/勤奋榜·曙光橙胶囊，无涟漪点击）+ 榜单卡（#1 金皇冠/#2 银/#3 铜徽章/其余黑体数字、40dp 圆头像（加载失败回退首字母）、昵称、主指标分/次+副指标）+ 吸底"我的排名"卡（primary/6 底+#rank+次数·均分；未上榜自动隐藏；列表底部预留 108dp 防遮挡）。`SpeechLeaderboardViewModel` 四象限（period×metric）缓存切换不重复请求 + 竞态防护（loadingKey 过滤过期响应）。
  - **验证**：Android `compileDebugKotlin`/`assembleDebug`/`testDebugUnitTest` 144/144（新增 `SpeechNotebookDtosTest` 4 例：画像小数/试用切片锁定数/伪字幕兜底/榜单缺省；`PronunciationUtilsTest` 8 例：zh 长日期四形态/得分档位/雷达维度零兜底/音素取前六；`PronunciationViewModelsTest` 3 例：notebook 派生+重入/四象限缓存切换/错误重试）；Web `tsc --noEmit` 零错误；**模拟器 E2E**（本地 dev server，`debug.baseUrl` 临时覆盖 `10.0.2.2`）：邮箱登录→弱项本（造数后画像 A2 级徽章/雷达像素校验紫+靛蓝/三统计 5·/θ/·1/横幅 5 句/列表+锁定卡"还有 2 条"）→闯关复习 (1/2)→(2/2) 流转+逐词渲染+末题"完成复习"+非会员 PRO 锁定态→达人榜四象限切换（空态/数据联动/副标题联动/#1 我的排名吸底卡"9 次评测·平均 53 分"/头像字母回退）；测试造数与角色临时升级已全部还原（USER/0 记录）。
  - **⚠️ 生产部署提醒**：`https://www.wxkzd.com` 尚未部署 notebook 路由——新 APK 在生产环境将无法加载弱项本（模拟器实测过 404→"弱项本数据加载失败"）。**后端先发版，再发 APK**。
  - **生产发布（2026-09-07 晚）**：后端 `03938dc`（notebook 路由 + leaderboard Bearer 修复）经 CI push master 自动部署生产并验证（匿名 401 / Bearer 全通 / evaluate 服务端闭环：TTS 音频实测 96.87 高分 + 不匹配音频 24.62 低分正确入弱项集）；Android `3574403` 推送 origin/master，`assembleRelease`（项目 keystore 签名，apksigner 校验 CN=Yuanlu/O=wxkzd）覆盖安装真机（华为 LYA-AL00，存储不足用 `pm trim-caches` 清理后成功，保数据保登录态）。真机生产冒烟：管理员账号弱项本全量数据与设计稿一致（B2 级/128 次评测/83 综合/108 词每分/3 条弱项：35·71·73 分），闯关页 (1/3) 渲染+中文翻译匹配，录音启动获 appops 佐证（RECORD_AUDIO 累计 4m03s）；⚠️ 华为 Android 10 的 uiautomator dump 对 Compose 状态更新有顽固滞后（录音态/返回导航均显示旧层级）且 bounds 偏移，自动化点击录音按钮不可靠——评测提交链路改由生产 API 直证。测试账号生产库现存 2 条评测记录（一次性账号，无需清理）。
...
### M9 — 个人中心 + 订阅 + 头像上传（~1 周）◐
- 资料/设置/订阅状态/头像直传 (OSS presign)/注销。（个人中心 + 头像 multipart 上传 ✅ 2026-09-04，订阅/注销待做）
