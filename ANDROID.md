...
- [x] **Phase 2 - M9: 个人中心 + 头像上传（旅程数据/里程碑/最近听过/账号与安全 + 编辑资料弹窗，复刻 Web /auth/personal-center；订阅部分仍待做）** (✅ 完成 2026-09-04，两端构建全绿 + 81 单测通过)
  ...
  - **UserProfileViewModel（Activity 作用域，Navigation 根创建）**：统管头部资料/旅程统计/周活动（本周↔上周切换）/成就/最近听过 + 编辑表单态；保存成功 merge 旧资料的 email/phone/role（PUT 响应无 User 嵌套）、`profileRevision` 自增——「我的」Tab 监听 revision 自动刷新用户卡（对齐 Web updateSession→fetchProfile）。
  - **主页（PersonalCenterScreen，新增 NavKey：`PersonalCenterNav`，「我的」菜单「个人中心」进入）**：头部用户卡（88dp 白描边头像 + 昵称 + 曙光橙「远行客·等级」badge + 座右铭回退 + 加入日期/属地）→ 四选项卡（下划线指示器 + fade/slide 切换）：**旅程数据**＝三统计卡（累计里程=时长×5km 一位小数/连续天数/词汇路标，ShimmerBox 骨架屏）+ 周活动面积图（Canvas 中点贝塞尔平滑 + 渐变填充 15%→0% + 白描边数据点 + TextMeasurer 轴刻度，本周/上周分段切换）；**里程碑**＝远路路图（Canvas：蜿蜒贝塞尔虚线 + PathMeasure 按里程实线裁剪 + 5 节点点亮/灰暗 + 冲线小旗 + 进度脉冲点 2s 循环）+ 成就墙（4 列网格前 8 枚，已解锁远青底/未解锁灰暗半透明，标题计数 + 全部查看占位）；**最近听过**＝最近 3 条（16:9 封面 + 标题 + 进度条/已听完/百分比，点击进播放器，骨架行）；**账号与安全**＝手机号/邮箱（脱敏 138****8000、a****@domain + 已验证绿标/未绑定警示 + 绑定胶囊占位）/登录密码三列表卡。
  ...
  - **验证**：Android `compileDebugKotlin`/`assembleDebug`/`testDebugUnitTest` 81/81（新增 `ProfileUtilsTest` 16 例 + `UserProfileViewModelTest` 10 例：加载/错误/周切换/表单回填默认值/昵称简介校验拦截/保存合并 revision/失败保弹窗/头像版本号；既有 LoginViewModelTest·ProfileViewModelTest 的 Fake 同步补新接口桩）；Web `tsc --noEmit` 零错误。
